...
<script type="text/javascript">
   ...
   $(document).ready(function(){
		...
		$("input#Number").mask("+7(999)999-99-99", {autoclear: false});
   });
</script>
<div class="Form">
			...
			<?endif?>
			<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="Order" onsubmit="ym(54496510, 'reachGoal', 'Consultation'); return true;">
				<?=bitrix_sessid_post()?>
				<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>"><?endif?>
								...
								<label>
									...
									<textarea cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
										...
										case "N":
											...
											?>
											<input placeholder="<?if($propertyID == "10"):?>+7 (***) ***-**-**<?elseif (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?> *" type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>"<?if($propertyID == "10"):?> id="Number"<?endif;?>>
											<? 
					...
					<?endif?>
					<div class="Submit">
						<div class="Btn Red"><input type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM")?>"></div>
