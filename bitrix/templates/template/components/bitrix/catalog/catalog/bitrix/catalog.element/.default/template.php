...
{
	if (!is_string($value)) return array('', false);
...
{
	if ($courseName !== REQUIRED_COURSE_NAME) return $planValue;
	...
	foreach ($leafTables as $table) {
		if (coursePlanHasRequiredStructure($table)) $validTables[] = $table;
	}
	if (!$validTables) return $planValue;

	$normalized = array();
	foreach ($validTables as $table) {
		$normalized[hash('sha256', coursePlanCellText($table))] = true;
	}
	if (count($normalized) > 1) return $planValue;
	if (!$wasWrapped && count($leafTables) === 1) return $planValue;
	return $validTables[0];
}
/* COURSE_PLAN_HELPER_END */
?>
