...
- Проект зарегистрирован в Bridge как `php-repository`.
- Обращение из привязанного Telegram-чата может быть передано владельцем в
  Bridge как repository-only задача: исполнитель готовит PR, а GitHub запускает
  обязательный check `verify`.
- Автоматическая задача не выполняет деплой и не меняет production, DNS или
  секреты. Для live-изменения нужен отдельный контролируемый этап.
- Яндекс Метрика используется только для агрегированной read-only статистики.
  Bitrix24 подключается через отдельный read-only inbound webhook; логин и
  пароль портала не являются API-интеграцией и в репозиторий не передаются.
